...
16. 产品界面必须是 **BitX.exe 桌面窗口（WebView2）**。禁止用系统浏览器打开工作台。localhost 只给窗口加载，不是给用户当网站。
17. 工作台外观对齐一代 BitX 窗口：菜单栏（文件/编辑/…）、活动栏、资源管理器、中央编辑器、右侧 BitX 对话、状态栏。禁止另做网页后台式控件台。不拷贝 `vs/*` 源码。
